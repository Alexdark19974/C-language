...
int get_line (struct key_value *k_v, int limit)
{
    ...
    {
        if (!isalnum(*tmp_w_ptr = c = getch()) && *tmp_w_ptr != '/' && *tmp_w_ptr != '*' && *tmp_w_ptr != '#' && *tmp_w_ptr != '_' && *tmp_w_ptr != '\"')
        {
            ungetch (*tmp_w_ptr);
            break;
        }
...
void hash_table(void)
{
	enum { EXAMPLE = 0, START, UNDEF, END };
    char instr = 0;
	char flag = 0;
...
void free_memory(void)
{
	for (int i = 0; my_strdup_collector[i] != NULL && i != my_strdup_collector_counter; i++)
    {
    	if (isalnum(*my_strdup_collector[i]))
        {
...
void start (char flag)
{
	...
	for (int i = 0; get_line(&k_v, CHARLIMIT) != EOF; i++)
    {
		if (flag == MANUAL)
		{
