...
/*
	the function for input "getchar" has been replaced with "scanf" to do the same work
	the implementation of "sscanf" is possible but requires deep code restructuring
	sscanf will be used in a different file.
*/
...
int getch(void)
{
	...
	if (bufp == 0)
	{
		while ((status = (scanf("%c", &line[i]))) && (line[i] != '\n'))
		{
			if (status == EOF)
        	{
            	return EOF;
